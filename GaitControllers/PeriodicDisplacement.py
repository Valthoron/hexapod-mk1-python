...
class PeriodicDisplacement(LegModelController):
    ...
    def update(self, dt, controls):
        ...
        if (control_mode_switch == 0):
            ...
            self.axis_leg_lift = Tools.toward(self.axis_leg_lift, 0.0, 4.0 * dt)
            self.axis_body_lift = Tools.toward(self.axis_body_lift, 0.0, 4.0 * dt)
        ...
        attitude_transform = numpy.matmul(mgen.rotation_around_z(self.twist), numpy.matmul(mgen.rotation_around_y(self.tilt_forward), mgen.rotation_around_x(self.tilt_side)))
        ...
        for i in range(6):
            ...
            tip_location_translated = numpy.add(HexapodConstants.TIP_LOCATION_DEFAULT[i], offset)
            tip_location = attitude_transform.dot(turn_transform.dot(tip_location_translated))
